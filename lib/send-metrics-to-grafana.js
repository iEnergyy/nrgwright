...
async function sendMetricsToGrafana() {
  ...
  try {
    ...
    // Add daily run count metrics
    const dailyRunMetrics = await getDailyRunMetrics();
    if (dailyRunMetrics) {
...
async function getDailyRunMetrics() {
  const metricsDir = path.join(process.cwd(), 'metrics');
  const today = new Date().toISOString().split('T')[0];
  const sessionsFile = path.join(metricsDir, `run-sessions-${today}.json`);
